...
$table = '';
    ...
    FROM 
        2_derivaciones a
    LEFT JOIN 
        2_derivaciones_hijos di ON a.ID_DERIVACION = di.ID_DERIVACION
    LEFT JOIN 
        pacientes b ON a.COD_RUTPAC = b.COD_RUTPAC
    LEFT JOIN 
        login g ON a.TENS = g.USUARIO
    LEFT JOIN 
        login c ON a.ENFERMERA = c.USUARIO
    LEFT JOIN 
        login med ON a.MEDICO = med.USUARIO
    LEFT JOIN 
        login adm ON a.ADMINISTRATIVA = adm.USUARIO
    WHERE 
        a.ESTADO_RN NOT IN ('Alta Paciente', 'Anulado')
...
$sql_details = array(
    'user' => 'redgescl_icrsredgescl',
    'pass' => '_z]*[nJOBGN-',
    'db'   => 'redgescl_cli_santiago',
    'host' => 'localhost'
);
