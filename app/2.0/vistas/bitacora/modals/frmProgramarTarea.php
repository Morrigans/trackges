...
$query_qrDerivacion= "SELECT * FROM $MM_oirs_DATABASE.2_bitacora WHERE ID_BITACORA = '$idBitacora'";
...
<script type="text/javascript">
	...
	$("#tablaDestinatariosAlarmas").load('2.0/vistas/bitacora/modals/tablaDestinatariosAlarmas.php?idBitacora='+idBitacora);
	...
	function fnProgramarTarea(idBitacora,usuario,idDerivacion){
		...
		$.ajax({
			type:"post",
			data:cadena,
			url:'2.0/vistas/bitacora/modals/programarTarea.php',
			success:function(r){
				if (r == 1) {
					Swal.fire({
					  position: 'top-end',
...
function fnCerrarProgramarTarea(idDerivacion){
	setTimeout(function (){ $('#dvTablaBitacora').load('2.0/vistas/bitacora/modals/tablaBitacora.php?idDerivacion='+idDerivacion); }, 1);//retardo PARA EVITAR dropdown
}
