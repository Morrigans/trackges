...
$query_qrCanastaPatologia = "SELECT * FROM $MM_oirs_DATABASE.canasta_patologia WHERE ID_CANASTA_PATOLOGIA='$canasta'";
...
$query_qrDepende = "SELECT * FROM $MM_oirs_DATABASE.canasta_patologia WHERE CODIGO_CANASTA_PATOLOGIA = '$codCanastaPatologia'";
...
$query_qrReglaDependencia = "SELECT * FROM $MM_oirs_DATABASE.derivaciones_canastas WHERE ID_DERIVACION = '$idDerivacion' AND ID_CANASTA = '$dependencia' AND ESTADO = 'activa'";
...
if ($repite == 'no') {
	...
	$query_qrReglaRepite = "SELECT * FROM $MM_oirs_DATABASE.derivaciones_canastas WHERE ID_DERIVACION = '$idDerivacion' AND ID_CANASTA = '$canasta'";
	$qrReglaRepite  = $oirs->SelectLimit($query_qrReglaRepite) or die($oirs->ErrorMsg());
	$totalRows_qrReglaRepite  = $qrReglaRepite ->RecordCount();

}else{
	//evalua si la canasta seleccionada ya existe y si esta activa, solo podra repetirse estando finalizada la primera.
	$query_qrReglaRepite = "SELECT * FROM $MM_oirs_DATABASE.derivaciones_canastas WHERE ID_DERIVACION = '$idDerivacion' AND ID_CANASTA = '$canasta' AND ESTADO = 'activa'";
	$qrReglaRepite  = $oirs->SelectLimit($query_qrReglaRepite) or die($oirs->ErrorMsg());
	$totalRows_qrReglaRepite  = $qrReglaRepite ->RecordCount();
}
...
//evalua que la si hay canastas activas en la derivacion, para advertir que no puede habilitarse esta canasta ya que no correo simultanea
	$query_qrReglaSimultanea = "SELECT * FROM $MM_oirs_DATABASE.derivaciones_canastas WHERE ID_DERIVACION = '$idDerivacion' AND ESTADO = 'activa'";
