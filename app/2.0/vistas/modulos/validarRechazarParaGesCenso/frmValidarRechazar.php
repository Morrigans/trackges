...
$('#dvInfoVentanasOpcionesCenso').load('2.0/vistas/modulos/infoVentanasOpciones.php?idDerivacion='+idDerivacion);
...
cadena = 'idDerivacion=' + idDerivacion +
		...
		$.ajax({
			type:"post",
			data:cadena,
			url:'2.0/vistas/modulos/validarRechazarParaGesCenso/agregarEstadoCenso.php',
			success:function(r){
				...
				if (r == 1) {
					...
					if (tipoUsuario == 1 || tipoUsuario == 2 || tipoUsuario == 3) {
						setTimeout(function (){ $('#dvTablaDerivaciones').load('2.0/vistas/inicio/inicioSupervisora/tablaDerivacionesApiCenso.php?estado=' + estado); }, 2001); 
					}
					if (tipoUsuario == 4) {// administrativa
						setTimeout(function (){ $('#contenido_principal').load('2.0/vistas/inicio/inicioAdministrativa/tablaDerivacionesApiCenso.php?estado=' + estado); }, 2001);
					}
					...
					if (tipoUsuario == 6) {//tens
						setTimeout(function (){ $('#contenido_principal').load('2.0/vistas/inicio/inicioTens/tablaDerivacionesApiCenso.php?estado=' + estado); }, 2001);
					}
	    		}else{
