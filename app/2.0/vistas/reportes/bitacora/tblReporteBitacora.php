...
if ($fecha != '' and $slEmisorBitacora != '' and $slAsuntoBitacora == '') {
  $query_qrReporteDiario= "SELECT * FROM $MM_oirs_DATABASE.2_bitacora WHERE AUDITORIA ='$fecha' AND SESION ='$slEmisorBitacora'";
  $qrReporteDiario = $oirs->SelectLimit($query_qrReporteDiario) or die($oirs->ErrorMsg());
  $totalRows_qrReporteDiario = $qrReporteDiario->RecordCount();
}

if ($fecha != '' and $slEmisorBitacora == '' and $slAsuntoBitacora != '') {
  $query_qrReporteDiario= "SELECT * FROM $MM_oirs_DATABASE.2_bitacora WHERE AUDITORIA ='$fecha' AND ASUNTO ='$slAsuntoBitacora'";
  $qrReporteDiario = $oirs->SelectLimit($query_qrReporteDiario) or die($oirs->ErrorMsg());
  $totalRows_qrReporteDiario = $qrReporteDiario->RecordCount();
}

if ($fecha != '' and $slEmisorBitacora != '' and $slAsuntoBitacora != '') {
  $query_qrReporteDiario= "SELECT * FROM $MM_oirs_DATABASE.2_bitacora WHERE AUDITORIA ='$fecha' AND ASUNTO ='$slAsuntoBitacora' AND SESION ='$slEmisorBitacora'";
  $qrReporteDiario = $oirs->SelectLimit($query_qrReporteDiario) or die($oirs->ErrorMsg());
  $totalRows_qrReporteDiario = $qrReporteDiario->RecordCount();
}
