...
$query_qrBuscaAlarmas = "SELECT * FROM $MM_oirs_DATABASE.2_alarmas WHERE ESTADO = 'desactivada' AND FECHA_ALARMA <= '$hoy' AND USUARIO_RECEPTOR = '$usuario' order by FECHA_ALARMA desc";
...
<table id="tAlarmasHistorial" class="table table-hover table-sm">
	<thead class="table-info">
		<tr align="center">
			<th><font>Emisor</font></th>
			<th><font>Derivacion</font></th>
	...
	<?php while (!$qrBuscaAlarmas->EOF) { 
		...
		<tr>
			...
			<td>
				<!-- <button class="btn btn btn-danger btn-xs" onclick="preguntaSiNofnDesprogramarTarea('<?php echo $idAlarma ?>','<?php echo $usuario ?>','<?php echo $qrBuscaBitacora->Fields('ID_DERIVACION') ?>')">Detener</button> -->
				<?php if ($cumplida=='') { ?>
					<button class="btn btn btn-success btn-xs" onclick="fnCheckearCumplidaHis('<?php echo $idAlarma ?>','<?php echo $usuario ?>','<?php echo $qrBuscaBitacora->Fields('ID_DERIVACION') ?>')"><i class="fas fa-check"></i> Realizada</button>
				<?php }else{ ?>
					<button class="btn btn btn-danger btn-xs" onclick="fnCheckearNoCumplidaHis('<?php echo $idAlarma ?>','<?php echo $usuario ?>','<?php echo $qrBuscaBitacora->Fields('ID_DERIVACION') ?>')"><i class="fas fa-times"></i> Quitar marca</button>
				<?php } ?>				
			</td>
		</tr>	
	<?php
		...
		$.ajax({
			...
			success:function(r){
				if (r == 1) {
					...
					setTimeout(function (){ $('#dvFrmAlarmasInicio').load('2.0/vistas/alarmas/modals/frmAlarmasInicio.php'); }, 1501);//retardo PARA EVITAR dropdown
	    	}
			}
		});
	}

	function fnCheckearCumplidaHis(idAlarma,usuario,idDerivacion){
		cadena = 'idAlarma=' + idAlarma +
				'&usuario=' + usuario +
		...
		$.ajax({
			...
			success:function(r){
				if (r == 1) {
					...
					setTimeout(function (){ $('#dvFrmAlarmasInicioHistorial').load('2.0/vistas/alarmas/modals/frmAlarmasInicioHistorial.php'); }, 1);//retardo PARA EVITAR dropdown
	    	}
			}
		});
	}

	function fnCheckearNoCumplidaHis(idAlarma,usuario,idDerivacion){
		cadena = 'idAlarma=' + idAlarma +
				'&usuario=' + usuario +
				'&idDerivacion=' + idDerivacion;
