...
$query_qrNotificación = "SELECT * FROM $MM_oirs_DATABASE.2_notificaciones where ESTADO='nuevo' and USUARIO = '$usuario' AND (ASUNTO!='Solicita gestion' AND ASUNTO!='Solicita Informacion') order by ID DESC";
...
// VERIFICA NUEVAS NOTIFICACIONES PARA EL PERFIL COORDINADOR
$query_qrNotificación2 = "SELECT * FROM $MM_oirs_DATABASE.2_notificaciones where ESTADO='nuevo' and USUARIO = '$usuario' AND (ASUNTO='Solicita gestion' OR ASUNTO='Solicita Informacion') order by ID DESC";
...
$query_qrAlarmas = "SELECT * FROM $MM_oirs_DATABASE.2_alarmas where ESTADO='activa' AND FECHA_ALARMA <= '$hoy' and USUARIO_RECEPTOR = '$usuario'  order by ID_ALARMA DESC";
...
?>
	...
	<nav class="main-header navbar navbar-expand navbar-white navbar-light">
	  <ul class="navbar-nav">
        ...
        <li class="nav-item">
            <a href="principal2.0.php" class="nav-link">Inicio</a> 
        </li>
        <li class="nav-item"> 
	  		...
	  		<li class="nav-item dropdown">
	  			<div class="row">
	  				...
	  				<!-- ALARMAS ACTIVAS E HISTORIAL -->
			  	  		<a class="nav-link" data-toggle="" href="#" onclick="$('#modalAlarmasInicio').modal('show'); $('#dvFrmAlarmasInicio').load('2.0/vistas/alarmas/modals/frmAlarmasInicio.php');" data-toggle="tooltip" data-placement="bottom" title="Tareas programadas">
		  			...
		  			<div class="">
				  	  	...
				  	  	<div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
				  	    	...
				  	    	<a href="#" class="dropdown-item" data-toggle="modal" data-target="#modalMensajeRecibido" onclick="fnVerMensajeRecibido(<?php echo $qrNotificación->Fields('ID'); ?>)">
				  	      		<i class="fas fa-envelope mr-2"></i> <?php echo utf8_encode($qrNotificación->Fields('ASUNTO')).' '.$qrNotificación->Fields('FOLIO'); ?><br>&ensp;&ensp;&ensp;<span class="text-muted text-sm"><?php echo date("d-m-Y",strtotime($qrNotificación->Fields('FECHA'))); ?></span><span class="float-right text-muted text-sm"><?php echo date("G:i",strtotime($qrNotificación->Fields('HORA'))); ?></span>
				  	    	</a>
					...
					<div class="">
			    	  	...
			    	  	<div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
			    	    	...
			    	    	<a href="#" class="dropdown-item" data-toggle="modal" data-target="#modalMensajeRecibido" onclick="fnVerMensajeRecibido(<?php echo $qrNotificación2->Fields('ID'); ?>)">
			    	      		<i class="fas fa-envelope mr-2"></i> <?php echo utf8_encode($qrNotificación2->Fields('ASUNTO')).' '.$qrNotificación2->Fields('FOLIO'); ?><br>&ensp;&ensp;&ensp;<span class="text-muted text-sm"><?php echo date("d-m-Y",strtotime($qrNotificación2->Fields('FECHA'))); ?></span><span class="float-right text-muted text-sm"><?php echo date("G:i",strtotime($qrNotificación2->Fields('HORA'))); ?></span>
			    	    	</a>
...
<script type="text/javascript">
	...
	function fnVerMensajeRecibido(id){
		$("#dvVerMensajeRecibido").load('2.0/notificaciones/mensajes/mensaje.php?id='+id);// carga mensaje en modalMensajeRecibido
	}
	function fnVerMensajeRecibidoIcrs(id){
		$("#dvVerMensajeRecibidoIcrs").load('2.0/notificaciones/mensajes/mensajeIcrs.php?id='+id);// carga mensaje en modalMensajeRecibido
	}
	function fnVerTodoMensajes(usuario){
		$("#dvVerTodoMensajesClinica").load('2.0/notificaciones/mensajes/todosLosMensajes.php?usuario='+usuario);// carga mensaje en modalMensajeRecibido
	}
	function fnVerTodoMensajesIcrs(usuario){
		$("#dvVerTodoMensajesIcrs").load('2.0/notificaciones/mensajes/todosLosMensajesIcrs.php?usuario='+usuario);// carga mensaje en modalMensajeRecibido
	}
</script>
