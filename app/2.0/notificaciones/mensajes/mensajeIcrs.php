...
$query_qrNotificación = "SELECT * FROM $MM_oirs_DATABASE.notificaciones_pp where ID='$id'";
...
<!-- <strong>Derivación:</strong> <a href="#" data-dismiss="modal"><span class="badge badge-warning" data-toggle="modal" data-target="#modalBitacora" onclick="fnfrmBitacora('<?php echo $qrNotificación->Fields('ID_DERIVACION') ?>')"><font size="3"><?php echo 'P0'.$qrNotificación->Fields('ID_DERIVACION'); ?></font></span></a></p><br> -->
<!-- <strong>mensaje:</strong> -->
<p><?php
	 echo utf8_encode(nl2br($mensaje)); 
	
	 if ($idRespuesta != '') {
	 	$query_qrRespuestas = "SELECT * FROM $MM_oirs_DATABASE.2_notificaciones where ID='$idRespuesta'";
...
<script type="text/javascript">
	...
	function fnEnviarRespuestaNotificacion(id){
		...
		$.ajax({
		    type: "POST",
		    url: "2.0/notificaciones/php/enviarRespuestaNotificacion.php",
		    data: cadena,
		    success: function(r) {
		    	Swal.fire({
					  position: 'top-end',
	...
	function fnCambiaEstadoNotificacion(id){
		...
		$.ajax({
		    type: "POST",
		    url: "2.0/notificaciones/php/cambiaEstadoNotificacionIcrs.php",
		    data: cadena,
		    success: function(r) {
		       $("#menuSuperior").load('2.0/notificaciones/navbar.php');// refresca menu superior
		    }
		});
	}
</script>