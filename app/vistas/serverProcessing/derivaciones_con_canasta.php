...
if ($tipo == 1 or $tipo == 2) {
	...
	ON a.ID_DERIVACION = derivaciones_canastas.ID_DERIVACION 

	LEFT JOIN canasta_patologia
	ON derivaciones_canastas.ID_CANASTA = canasta_patologia.ID_CANASTA_PATOLOGIA 
...
$columns = array(
						...
						if ($row[2]=='pendiente') {
							return ' 
								<div class="btn-group">
									...
									<a class="dropdown-item" href="#" data-toggle="modal" data-target="#modalAceptarCaso" onclick="fnfrmAceptarCaso('.$d.')">Aceptar Caso</a>
						...
						if ($row[2]=='prestador') {
							return ' 
								<div class="btn-group">
									...
									<a class="dropdown-item" href="#" data-toggle="modal" data-target="#modalContactarPaciente" onclick="fnfrmContactarPaciente('.$d.')">Contactar Paciente</a>
									<a class="dropdown-item" href="#" data-toggle="modal" data-target="#modalDerivacion" onclick="fnfrmDetalleDerivacion('.$d.')">Etapas/Canastas</a>
									<a class="dropdown-item" href="#" data-toggle="modal" data-target="#modalReasignarCaso" onclick="fnfrmReasignarCaso('.$d.')">Reasignar Caso</a>
									<a class="dropdown-item" href="#" data-toggle="modal" data-target="#modalCerrarCaso" onclick="fnfrmCerrarCaso('.$d.')">Cerrar Caso</a>
									<div class="dropdown-divider"></div>
										<a class="dropdown-item" href="#" data-toggle="modal" data-target="#modalBitacora" onclick="fnfrmBitacora('.$d.')">Bitacora</a> 
	...
	array( 'db' => 'FECHA_CANASTA',  'dt' => 4 , "formatter" => function($d, $row) {
						$fechaLatino = date("d-m-Y",strtotime($row[4]));
						return '<html><font size="3">'.$fechaLatino.'</font></html>';
						}),
	...
	array( 'db' => 'CODIGO_CANASTA_PATOLOGIA',  'dt' => 9),
	array( 'db' => 'DESC_CANASTA_PATOLOGIA',  'dt' => 10),
	array( 'db' => 'PREVISION',  'dt' => 11 ),
	array( 'db' => 'NOMBRE_PROFESIONAL',  'dt' => 12),
	array( 'db' => 'ID_DERIVACION',  'dt' => 13, "formatter" => function($d, $row) {return '<html><font size="3">'.$row[0].'</font></html>';}),
