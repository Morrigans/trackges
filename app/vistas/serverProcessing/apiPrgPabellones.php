	...
	$table = <<<EOT
	 (
	  SELECT 
	   ...
	   api_prog_pabellones.ESTADO AS ESTADO_PABELLON,
	   api_prog_pabellones.ID_RESERVA,
	   api_prog_pabellones.FECHA_RESERVA,
	   api_prog_pabellones.ESTADO_PROGRAMACION,
	   api_prog_pabellones.ID,
	   api_prog_pabellones.MOTIVO_RETRASO
...
$columns = array(
	array( 'db' => 'ID_DERIVACION', 'dt' => 0 , "formatter" => function($d, $row) {
							$estado = $row[12];  // Assuming 'ESTADO_PABELLON' is at index 11
									$additionalOption = '';

									if ($estado == 'retrasado') {
										$additionalOption = '<a class="dropdown-item" href="#" data-toggle="modal" data-target="#modalMotRetPabPrg" onclick="fnfrmMotRetPabPrg('.$row[16].')">Motivo retraso</a>';
									}
							return ' 
								<div class="btn-group">
	...
	array( 'db' => 'COD_PRESTACION',  'dt' => 9 , "formatter" => function($d, $row) {return '<html><font size="3">'.$d.'</font></html>';}),

	array( 'db' => 'CIRUGIA',  'dt' => 10, "formatter" => function($d, $row) {return '<html><font size="3">'.$d.'</font></html>';}),

	array( 'db' => 'ESTADO_PABELLON',  'dt' => 11, "formatter" => function($d, $row) {return '<html><font size="3">'.$d.'</font></html>';}),

	array( 'db' => 'ESTADO_PROGRAMACION',  'dt' => 12, "formatter" => function($d, $row) {return '<html><font size="3">'.$d.'</font></html>';}),

	array( 'db' => 'NOMBRE_PROFESIONAL',  'dt' => 13, "formatter" => function($d, $row) {return '<html><font size="3">'.$d.'</font></html>';}),
	
	array( 'db' => 'MOTIVO_RETRASO',  'dt' => 14, "formatter" => function($d, $row) {return '<html><font size="2">'.$d.'</font></html>';}), 

	array( 'db' => 'ID_DERIVACION',  'dt' => 15, "formatter" => function($d, $row) {return '<html><font size="2">'.$row[0].'</font></html>';}),

	array( 'db' => 'ID',  'dt' => 16, "formatter" => function($d, $row) {return '<html><font size="2">'.$d.'</font></html>';}),  
