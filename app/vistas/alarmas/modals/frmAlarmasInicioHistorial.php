...
$query_qrBuscaAlarmas = "SELECT * FROM $MM_oirs_DATABASE.alarmas WHERE ESTADO = 'desactivada' AND FECHA_ALARMA <= '$hoy' AND USUARIO_RECEPTOR = '$usuario' order by FECHA_ALARMA desc";
...
<script type="text/javascript">
	...
	function fnCheckearCumplidaHis(idAlarma,usuario,idDerivacion){
		...
		$.ajax({
			type:"post",
			data:cadena,
			url:'vistas/alarmas/php/marcaCumplida.php',
			success:function(r){
				if (r == 1) {
					Swal.fire({
					  position: 'top-end',
					...
					setTimeout(function (){ $('#dvFrmAlarmasInicioHistorial').load('vistas/alarmas/modals/frmAlarmasInicioHistorial.php'); }, 1);//retardo PARA EVITAR dropdown
	    	}
			}
		});
	}
	...
	function fnCheckearNoCumplidaHis(idAlarma,usuario,idDerivacion){
		...
		$.ajax({
			type:"post",
			data:cadena,
			url:'vistas/alarmas/php/marcaNoCumplida.php',
			success:function(r){
				if (r == 1) {
					Swal.fire({
					  position: 'top-end',
