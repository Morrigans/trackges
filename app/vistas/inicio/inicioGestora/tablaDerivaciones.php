...
require_once('modals/aceptarCaso/modalAceptarCasoGestora.php');
require_once('modals/cerrarCaso/modalCerrarCasoGestora.php');
require_once('modals/reasignarCaso/modalReasignarCasoGestora.php');
require_once('modals/asignarPrestadorCaso/modalAsignarPrestadorCasoGestora.php');
require_once('modals/derivacion/modalDerivacionGestora.php');   
require_once('modals/informacionPaciente/modalEditaInformacionPacienteGestora.php');
...
if ($estado == '') {
	$query_qrDerivacion = "SELECT ID_DERIVACION,N_DERIVACION,RUT_PACIENTE,NOMBRE_PACIENTE,NOMBRE_PROFESIONAL,PREVISION,ESTADO,FECHA_DERIVACION,DESC_PATOLOGIA,DESC_TIPO_PATOLOGIA,DESC_PATOLOGIA,REASIGNADA,RUT_PRESTADOR,DESC_PRESTADOR FROM $MM_oirs_DATABASE.vista_derivaciones where ESTADO!='cerrada' AND RUT_PROFESIONAL = '$usuario'"; 
	$qrDerivacion = $oirs->SelectLimit($query_qrDerivacion) or die($oirs->ErrorMsg());
	$totalRows_qrDerivacion = $qrDerivacion->RecordCount();

}if ($estado == 'pendiente' or $estado =='aceptada' or $estado =='prestador' or $estado =='cerrada'){
	$query_qrDerivacion = "SELECT ID_DERIVACION,N_DERIVACION,RUT_PACIENTE,NOMBRE_PACIENTE,NOMBRE_PROFESIONAL,PREVISION,ESTADO,FECHA_DERIVACION,DESC_PATOLOGIA,DESC_TIPO_PATOLOGIA,DESC_PATOLOGIA,REASIGNADA,RUT_PRESTADOR FROM $MM_oirs_DATABASE.vista_derivaciones where ESTADO ='$estado' AND RUT_PROFESIONAL = '$usuario'"; 
	$qrDerivacion = $oirs->SelectLimit($query_qrDerivacion) or die($oirs->ErrorMsg());
	$totalRows_qrDerivacion = $qrDerivacion->RecordCount();
}
if($estado != 'pendiente' and $estado != 'aceptada' and $estado != 'prestador' and $estado != 'cerrada' and $estado !='' and $vencidas!='vencidas'){ 

	$query_qrDerivacion = "SELECT
	DISTINCT($MM_oirs_DATABASE.vista_derivaciones.ID_DERIVACION),
	$MM_oirs_DATABASE.vista_derivaciones.N_DERIVACION,
	$MM_oirs_DATABASE.vista_derivaciones.ESTADO,
	$MM_oirs_DATABASE.vista_derivaciones.FECHA_DERIVACION,
	$MM_oirs_DATABASE.derivaciones_canastas.FECHA_LIMITE,
	$MM_oirs_DATABASE.vista_derivaciones.RUT_PACIENTE,
	$MM_oirs_DATABASE.vista_derivaciones.CODIGO_TIPO_PATOLOGIA,
	$MM_oirs_DATABASE.vista_derivaciones.CODIGO_PATOLOGIA,
	$MM_oirs_DATABASE.vista_derivaciones.PREVISION, 
	$MM_oirs_DATABASE.vista_derivaciones.REASIGNADA,
	$MM_oirs_DATABASE.vista_derivaciones.RUT_PROFESIONAL,
	$MM_oirs_DATABASE.vista_derivaciones.NOMBRE_PACIENTE,
	$MM_oirs_DATABASE.vista_derivaciones.DESC_TIPO_PATOLOGIA,
	$MM_oirs_DATABASE.vista_derivaciones.DESC_PATOLOGIA,
	$MM_oirs_DATABASE.vista_derivaciones.NOMBRE_PROFESIONAL
	FROM $MM_oirs_DATABASE.vista_derivaciones, $MM_oirs_DATABASE.derivaciones_canastas
	where
	$MM_oirs_DATABASE.vista_derivaciones.ID_DERIVACION =  $MM_oirs_DATABASE.derivaciones_canastas.ID_DERIVACION and
	$MM_oirs_DATABASE.derivaciones_canastas.FECHA_LIMITE<='$estado' and 
	...
	$MM_oirs_DATABASE.vista_derivaciones.ESTADO !='cerrada' AND 
	$MM_oirs_DATABASE.vista_derivaciones.CODIGO_TIPO_PATOLOGIA = '1' and
	$MM_oirs_DATABASE.derivaciones_canastas.ESTADO != 'finalizada' AND
	$MM_oirs_DATABASE.vista_derivaciones.RUT_PROFESIONAL = '$usuario'";
	$qrDerivacion = $oirs->SelectLimit($query_qrDerivacion) or die($oirs->ErrorMsg());
	$totalRows_qrDerivacion = $qrDerivacion->RecordCount();
}
...
<table id="tPacientesDerivados" class="table table-bordered table-striped table-hover table-sm">
	...
	while (!$qrDerivacion->EOF) {
		...
		$query_qrDerivacionCanasta= "SELECT FECHA_LIMITE, FECHA_CANASTA,CODIGO_CANASTA_PATOLOGIA FROM $MM_oirs_DATABASE.derivaciones_canastas WHERE ID_DERIVACION = '$idDerivacion' and ESTADO ='activa' order by FECHA_LIMITE ASC";
		...
		$query_qrPrestadorAsig= "SELECT RUT_PRESTADOR FROM $MM_oirs_DATABASE.derivaciones_canastas WHERE ID_DERIVACION = '$idDerivacion'";
	...
	?>
		<tr>
			<td>
				<div class="btn-group">
					...
					<div class="dropdown-menu" role="menu">
						...
						<a class="dropdown-item" target="_BLANK" href="vistas/inicio/inicioSupervisora/modals/enviaInfoPacACorreo/detallePacientePdf.php?idDerivacion=<?php echo $idDerivacion ?>&codRutPac=<?php echo $codRutPac ?>">Genera Pdf</a> 
					</div>
				</div>
			</td>
			<td><span class="badge badge-warning">
					<font size="2">
					<?php 
						if ($totalRows_qrBuscaProgramacionTarea>0) {?>
			...
			<td><a href="#" data-toggle="modal" data-target="#modalEditaInformacionPacienteGestora" onclick="fnFrmEditaInformacionPacienteGestora('<?php echo $qrDerivacion->Fields('ID_DERIVACION') ?>')"><font size="1"><b><?php echo utf8_encode(strtoupper($qrDerivacion->Fields('NOMBRE_PACIENTE'))); ?></b></font></a></td>
					...
					<?php 
						if ($totalRows_qrDerivacionCanasta == 0) {
							...
							while (!$qrDerivacionEtapa->EOF) {
								...
								$query_qrEtapaPatologia= "SELECT DESC_ETAPA_PATOLOGIA FROM $MM_oirs_DATABASE.etapa_patologia WHERE CODIGO_ETAPA_PATOLOGIA = '$codEtapaPatologia'";
			...
			<td>
				<font size="2">
					<?php  
					$i=1;
					if ($qrDerivacion->Fields('DESC_TIPO_PATOLOGIA')=='CAEC') {
						$rutPrestador = $qrDerivacion->Fields('RUT_PRESTADOR');

						if ($rutPrestador == '') {
							echo $i.'.- '.'Sin prestador.'.'</br>';
						}else{
							echo $i.'.- '.utf8_encode($qrDerivacion->Fields('DESC_PRESTADOR')).'.</br>';
						}
					}
					while (!$qrPrestadorAsig->EOF) {
						$rutPrestador = $qrPrestadorAsig->Fields('RUT_PRESTADOR');

						$query_qrPrestador= "SELECT DESC_PRESTADOR FROM $MM_oirs_DATABASE.prestador WHERE RUT_PRESTADOR = '$rutPrestador'";
