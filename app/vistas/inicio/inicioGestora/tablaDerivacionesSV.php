...
if ($estado == '') {
	...
	$query_qrDerivacion = "SELECT * FROM $MM_oirs_DATABASE.derivaciones WHERE ESTADO = '$estado' AND ENFERMERA = '$usuario'";
	$qrDerivacion = $oirs->SelectLimit($query_qrDerivacion) or die($oirs->ErrorMsg());
	$totalRows_qrDerivacion = $qrDerivacion->RecordCount();
}
if($estado != 'pendiente' and $estado != 'aceptada' and $estado != 'prestador' and $estado != 'cerrada' and $estado !='' and $vencidas!='vencidas'){ 
	...
	DISTINCT($MM_oirs_DATABASE.derivaciones.ID_DERIVACION),
...
<table id="tPacientesDerivados" class="table table-bordered table-striped table-hover table-sm">
	...
	while (!$qrDerivacion->EOF) {
		...
		$query_qrPrestadorAsig= "SELECT * FROM $MM_oirs_DATABASE.derivaciones_canastas WHERE ID_DERIVACION = '$idDerivacion'";
		...
		$query_qrDerivacionCanasta= "SELECT * FROM $MM_oirs_DATABASE.derivaciones_canastas WHERE ID_DERIVACION = '$idDerivacion' and ESTADO ='activa' order by FECHA_LIMITE ASC";
	...
	?>
		<tr>
			...
			<td><span class="badge badge-warning">
					<font size="2">
					<?php 
						$query_qrBuscaProgramacionTarea= "SELECT * FROM $MM_oirs_DATABASE.bitacora WHERE ID_DERIVACION = '$idDerivacion' AND PROGRAMADO = 'si'";
			...
			<td>
				<font size="2">
					<?php
					if ($qrDerivacionCanasta->Fields('FECHA_LIMITE') == '0000-00-00' or $qrDerivacionCanasta->Fields('FECHA_LIMITE') == null or $qrTipoPatologia->Fields('DESC_TIPO_PATOLOGIA') == 'CAEC' or $totalRows_qrDerivacionCanasta == 0) {
						echo 'Sin Limite';
					}else{
			...
			<td><a href="#" data-toggle="modal" data-target="#modalEditaInformacionPacienteGestora" onclick="fnFrmEditaInformacionPacienteGestora('<?php echo $qrDerivacion->Fields('ID_DERIVACION') ?>')"><font size="1"><b><?php echo utf8_encode(strtoupper($qrPaciente->Fields('NOMBRE'))); ?></b></font></a></td>
			...
			<td>
				<font size="2">
					<?php  
					$i=1;
					if ($qrDerivacion->Fields('CODIGO_TIPO_PATOLOGIA')==2) {
						...
						if ($rutPrestador == '') {
							...
							echo utf8_encode($qrPrestador->Fields('DESC_PRESTADOR'));
						}
					}else{
