...
if ($tipoProfiesional == 1 or $tipoProfiesional == 2) {
	$query_qrDerivacion = "SELECT * FROM $MM_oirs_DATABASE.derivaciones WHERE ESTADO_ANULACION ='anulado'";
	$qrDerivacion = $oirs->SelectLimit($query_qrDerivacion) or die($oirs->ErrorMsg());
	$totalRows_qrDerivacion = $qrDerivacion->RecordCount();
}
if ($tipoProfiesional == 3) {
	$query_qrDerivacion = "SELECT * FROM $MM_oirs_DATABASE.derivaciones WHERE ESTADO_ANULACION ='anulado' and ENFERMERA='$usuario'";
	$qrDerivacion = $oirs->SelectLimit($query_qrDerivacion) or die($oirs->ErrorMsg());
	$totalRows_qrDerivacion = $qrDerivacion->RecordCount();
}
...
<script type="text/javascript">
		...
		$(function () {
		    $('#tPacientesAnulados').DataTable({
		      ...
		      "serverSide": true,
		      "ajax": {
		                 "url": "vistas/serverProcessing/anuladas.php",
		                 data: function (d) {
