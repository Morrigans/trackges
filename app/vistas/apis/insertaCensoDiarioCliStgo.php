...
foreach ($array as $value) {
    ...
    $query_qrDerivacion = "SELECT * FROM $MM_oirs_DATABASE.derivaciones WHERE ID_PACIENTE = '$idPaciente' AND ESTADO <> 'cerrada' AND ESTADO_ANULACION = 'activo'";
    ...
    $insertSQLHistorico = sprintf("INSERT INTO $MM_oirs_DATABASE.api_censo_historico (rut_paciente, id_admision, fecha_censo, fecha_ingreso, dias_ingresado, codigo_prestacion, diagnostico, nombre_convenio, ley_urgencia, fecha_foto, fecha_registro, ID_DERIVACION, FOLIO) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    $query_qrCenso = "SELECT * FROM $MM_oirs_DATABASE.api_censo WHERE id_admision = '$idAdmision'";
    $qrCenso = $oirs->SelectLimit($query_qrCenso) or die($oirs->ErrorMsg());
    $totalRows_qrCenso = $qrCenso->RecordCount();

    if ($totalRows_qrCenso > 0) {
        $updateSQL = sprintf("UPDATE $MM_oirs_DATABASE.api_censo SET dias_ingresado=%s, ley_urgencia=%s, fecha_foto=%s, fecha_registro=%s, fecha_censo=%s WHERE id_admision= '$idAdmision'",
            GetSQLValueString($value['dias_ingresado'], "int"),
            GetSQLValueString($value['ley_urgencia'], "text"),
            GetSQLValueString($value['fecha_foto'], "date"),
            GetSQLValueString($auditoria, "date"),
    ...
    $insertSQLDiario = sprintf("INSERT INTO $MM_oirs_DATABASE.api_censo (rut_paciente, id_admision, fecha_censo, fecha_ingreso, dias_ingresado, codigo_prestacion, diagnostico, nombre_convenio, ley_urgencia, fecha_foto, fecha_registro, ID_DERIVACION, FOLIO, ESTADO) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
        GetSQLValueString($value['rut_paciente'], "text"),
        GetSQLValueString($value['id_admision'], "text"),
        GetSQLValueString($value['fecha_censo'], "date"),
        GetSQLValueString($value['fecha_ingreso'], "date"),
...
$query_qrCargaAnterior = ("SELECT FOLIO, rut_paciente, id_admision, codigo_prestacion FROM api_censo_historico WHERE fecha_registro = '$cargaAnterior'");
...
$totalRows_qrCargaAnterior = $qrCargaAnterior->RecordCount();
 ...
 while (!$qrCargaAnterior->EOF) { 
  ...
  $nomPaciente2 = $qrPacientes2->Fields('NOMBRE');
    ...
    $query_qrBuscaFolioAnulado = ("SELECT * FROM api_censo_historico WHERE FOLIO = '$folioAnterior' and fecha_registro = '$auditoria'");
