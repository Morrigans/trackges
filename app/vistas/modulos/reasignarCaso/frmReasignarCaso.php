...
<script>
	...
	$('#dvInfoVentanasOpcionesReasignarCaso').load('vistas/modulos/infoVentanasOpciones.php?idDerivacion='+idDerivacion);
...
function fnReasignarCaso(idDerivacion, tipoUsuario){
	...
	cadena = 'idDerivacion=' + idDerivacion +
		...
		$.ajax({
			type:"post",
			data:cadena,
			url:'vistas/modulos/reasignarCaso/reasignarCaso.php',
			success:function(r){
				if (r == 1) {
					Swal.fire({
					  position: 'top-end',
