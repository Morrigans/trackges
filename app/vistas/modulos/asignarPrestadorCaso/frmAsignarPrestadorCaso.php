...
$('#dvInfoVentanasOpcionesAsignaPrestador').load('vistas/modulos/infoVentanasOpciones.php?idDerivacion='+idDerivacion);
...
function fnConsultaSiTieneModuloPrestador(){
	...
	$.ajax({
			type:"post",
			data:cadena,
			url:'vistas/modulos/asignarPrestadorCaso/consultaSiTieneModuloPrestador.php', 
			success:function(r){
...
function fnAsignarPrestadorCaso(idDerivacion,tipoUsuario,canasta){
	...
	cadena = 'idDerivacion=' + idDerivacion +
		...
		$.ajax({
			type:"post",
			data:cadena,
			url:'vistas/modulos/asignarPrestadorCaso/asignarPrestadorCaso.php',
			success:function(r){
				if (r == 1) {
					Swal.fire({
					  position: 'top-end',
