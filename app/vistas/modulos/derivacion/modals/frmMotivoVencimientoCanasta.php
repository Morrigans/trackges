        ...
        <div class="card-body">
        	<div class="row">
        		...
        		<div class="input-group mb-3 col-sm-5">
	  			...
	  			<div id="dvSlMotivo" class="input-group mb-3 col-sm-4">
			        <div class="input-group-prepend">
		    	...
		    	<div class="col-sm-1">	
				    <button type="button" class="btn btn-default" onclick="fnCancelaMotivoVencimientoCanasta('<?php echo $idDerivacion ?>')">Cancelar</button>
	  			</div> 
	  			<div class="col-sm-2">					   
				    <button type="button" class="btn btn-danger" onclick="fnMotivoVencimientoCanasta('<?php echo $idDerivacion ?>','<?php echo $idEtapaPatologia ?>','<?php echo $codEtapaPatologia ?>','<?php echo $canasta ?>','<?php echo $prestador ?>')">Guardar Motivo</button>
	  			</div> 
  			</div>    
	  	</div>
...
function fnCancelaMotivoVencimientoCanasta(idDerivacion){
...
function fnMotivoVencimientoCanasta(idDerivacion,idEtapaPatologia,codEtapaPatologia,canasta,prestador){
	...
	if (slMotivoFinCanasta == '' || comentarioCambioEstadoCanasta == '') {
		Swal.fire({
		  position: 'top-end',
		  icon: 'warning',
	...
	cadena = 'idDerivacion=' + idDerivacion +
		...
		$.ajax({
			type:"post",
			data:cadena,
			url:'vistas/inicio/inicioSupervisora/modals/derivacion/modals/guardaMotivoVencimientoCanasta.php',
			success:function(r){
				if (r == 1) {
					swal("Todo bien!", "Motivo guardado con exito", "success");
				$('#dvfrmDetalleDerivacion').load('vistas/inicio/inicioSupervisora/modals/derivacion/frmDetalleDerivacion.php?idDerivacion=' + idDerivacion);
	    	}
