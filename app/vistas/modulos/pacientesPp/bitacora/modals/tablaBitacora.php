...
$query_qrBitacora = "SELECT * FROM $MM_oirs_DATABASE.bitacora_pp WHERE ID_DERIVACION = '$idDerivacion' AND (ESTADO IS NULL or ESTADO = '') AND (COMPARTIDO_ICRS IS NULL OR COMPARTIDO_ICRS='si') order by ID_BITACORA desc";
...
<html>
	<body>
      <div class="card-body">
      	<div id="dvAdjuntarDocumentoPp"></div>
      	<br>
      			...
      			<table class="table" id="tbitacora">
      				...
      				<tbody>
	      				<?php while (!$qrBitacora->EOF) {
	      					...
	      					WHERE 
	      					ID_DERIVACION = '$idDerivacion' and 
	      					PRESTADOR_ORIGEN = '49'";
	      					...
	      					if($compartidoIcrs=='si' or $sesion == 'ICRS'){
	      					...
	      					<th style="width:30%">
 								...
 								?>
	      						...
	      						<?php if($ruta=='' and $qrBitacora->Fields('SESION') == $usuario){ ?>
	      							<a href="#" onclick="$('#dvAdjuntarDocumentoPp').load('vistas/modulos/pacientesPp/bitacora/adjuntaDoc/adjuntaDocumento.php?idBitacora='+<?php echo $idBitacora?>+'&idDerivacion='+<?php echo $idDerivacion?>)"><span class="badge badge-warning"><i class="fas fa-paperclip"></i></span></a>
	      						<?php }
	      						
	      						//si tiene adjunto y es del prestador solo muestra el ver documento, el eliminar documento se inactiva
	      						if($ruta!='' and $qrBitacora->Fields('SESION') == 'ICRS'){ ?>
									<span class=""><a target="_blank" class="btn btn-xs btn-success" href="https://icrs.redges.cl/vistas/bitacora/adjuntaDoc/<?php echo $ruta; ?>" ><i class="far fa-file-pdf"></i></a></span>
	      						<?php } 
	      						...
	      						if ($qrBitacora->Fields('SESION') == $usuario) {
		      						...
		      						if($ruta!='' and $qrBitacora->Fields('SESION') != null){ ?>
										[<span class=""><a target="_blank" class="btn btn-xs btn-success" href="vistas/modulos/pacientesPp/bitacora/adjuntaDoc/<?php echo $ruta; ?>" ><i class="far fa-file-pdf"></i></a></span>
	      						...
	      						if ($qrDerivacionPp->Fields('PRESTADOR_ORIGEN') == 49) {
	      						 ...
	      						 if ($qrBitacora->Fields('SESION') != null and $qrBitacora->Fields('COMPARTIDO') != 'si' and $qrBitacora->Fields('SESION') == $usuario) {?>
      								...
      								<select style="display: none;" name="slElijePrestador<?php echo $idBitacora ?>" id="slElijePrestador<?php echo $idBitacora ?>" onchange="preguntarSiNoCompartirRegistroBitacora('<?php echo $idBitacora ?>','<?php echo $ruta ?>','<?php echo $idDerivacion ?>','<?php echo trim(preg_replace('/\s+/', ' ',$comentarioBitacora)); ?>','<?php echo utf8_encode($asunto) ?>','<?php echo $rutaAudio ?>','<?php echo $qrDerivacionPp->Fields('PRESTADOR_ORIGEN') ?>')">
					                    ...
					                    <?php 
					                    	...
					                    	$query_qrPrestador= "SELECT * FROM $MM_oirs_DATABASE.prestador WHERE RUT_PRESTADOR = '$gestorRed'";
...
function fnProgramarTarea(idBitacora,idDerivacion){
	...
	$('#dvfrmProgramarTarea').load('vistas/modulos/pacientesPp/bitacora/modals/frmProgramarTarea.php?idBitacora='+idBitacora + '&idDerivacion='+idDerivacion);
}
...
function eliminaRegistoBitacora(idBitacora,idDerivacion){
	...
	$.ajax({
		type:"post",
		data:cadena,
		url:'vistas/modulos/pacientesPp/bitacora/modals/eliminaRegistoBitacora.php',
		success:function(r){
			...
			if (r == 1) {
				...
				setTimeout(function (){ $('#dvTablaBitacoraPp').load('vistas/modulos/pacientesPp/bitacora/modals/tablaBitacora.php?idDerivacion='+idDerivacion); }, 1501);//retardo PARA EVITAR dropdown
    	}
		}
	});
}
...
function fnDesprogramarTarea(idBitacora,usuario,idDerivacion){
	...
	$.ajax({
		type:"post",
		data:cadena,
		url:'vistas/modulos/pacientesPp/bitacora/modals/desprogramarTarea.php',
		success:function(r){
			if (r == 1) {
				Swal.fire({
				  position: 'top-end',
...
$(function () {
	...
	function preguntarSiNoCompartirRegistroBitacora(idBitacora,ruta,idDerivacion,comentarioBitacora,asunto,rutaAudio,gestorRed){
			Swal.fire({
		  title: 'Estas Seguro?',
		  ...
		  if (result.isConfirmed) {
		  	fnCompartirRegistroBitacora(idBitacora,ruta,idDerivacion,comentarioBitacora,asunto,rutaAudio,gestorRed)
		    Swal.fire(
		      'Compartido!',
...
function fnCompartirRegistroBitacora(idBitacora,ruta,idDerivacion,comentarioBitacora,asunto,rutaAudio,gestorRed){
	cadena = 'idBitacora=' + idBitacora+
			  '&ruta='+ ruta +
  ...
  $.ajax({
		type: "POST",
	    url: "vistas/modulos/pacientesPp/bitacora/modals/compartirMensajeGestorRed.php",
	    data: cadena,
		success:function(r){
    ...
    $.ajax({
  		type: "POST",
  	    url: "vistas/modulos/pacientesPp/bitacora/modals/estadoCompartido.php",
  	    data: cadena,
  		success:function(r){
          	$('#dvTablaBitacoraPp').load('vistas/modulos/pacientesPp/bitacora/modals/tablaBitacora.php?idDerivacion=' + idDerivacion);
        	if (r=1) {
        		// swal("Todo bien!", "se compartio registro con el Gestor de red", "success");
        	}
	...
	function fnQuitarEliminaAdjunto(idBitacora,ruta,idDerivacion){
	  ...
	  $.ajax({
	      type: "POST",
	      url: "vistas/modulos/pacientesPp/bitacora/adjuntaDoc/docs/eliminaAdjunto.php",
	      data: cadena,
	      success: function(r) {
	        $('#dvTablaBitacoraPp').load('vistas/modulos/pacientesPp/bitacora/modals/tablaBitacora.php?idDerivacion=' + idDerivacion);
	...
	function fnQuitarEliminaAudio(idBitacora,ruta,idDerivacion){
	  ...
	  $.ajax({
	      type: "POST",
	      url: "vistas/modulos/pacientesPp/bitacora/modals/audios/eliminaAudio.php",
	      data: cadena,
	      success: function(r) {
	        $('#dvTablaBitacoraPp').load('vistas/modulos/pacientesPp/bitacora/modals/tablaBitacora.php?idDerivacion=' + idDerivacion);
