...
$query_select = ("SELECT max(ID_BITACORA) as ID_BITACORA FROM $MM_oirs_DATABASE.bitacora_pp");
...
if ($asunto == 'Solicita gestión' or $asunto == 'Solicita Información') {
    ...
    if ($tipoUsuario == '3') { // si es tipo gestora, notificacion llega a su team de gestion (administrativa y tens)
        ...
        $query_qrBuscaTeam = "
            ...
            LEFT JOIN team_gestion
                ON derivaciones_pp.ID_DERIVACION = team_gestion.ID_DERIVACION
                
            LEFT JOIN login
                ON team_gestion.ID_PROFESIONAL = login.ID

            WHERE 
            derivaciones_pp.ID_DERIVACION = '$idDerivacion'
        ...
        while (!$qrBuscaTeam->EOF) {
            ...
            if ($receptor != '') {
                $insertSQL2 = sprintf("INSERT INTO $MM_oirs_DATABASE.notificaciones_pp (ID_DERIVACION, USUARIO, ASUNTO, MENSAJE, FECHA, HORA, ESTADO, USUARIO_EMISOR, ID_BITACORA) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
    ...
    if ($tipoUsuario == '1' or $tipoUsuario == '2') { // si tipo es administrador o supervisor notificacion llega a gestora y team gestion
        ...
        $query_qrBuscaGestora = "
            ...
            LEFT JOIN login
            ON derivaciones_pp.ENFERMERA = login.ID
    ...
    if ($tipoUsuario == '4') { // si tipo es administrativa notificacion llega a gestora y A TENS (6)
        ...
        $query_qrBuscaTeam = "
            ...
            WHERE 
            derivaciones_pp.ID_DERIVACION = '$idDerivacion' AND 
            login.TIPO = '6'
