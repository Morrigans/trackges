        ...
        <div class="card-body">
        	<div class="row">
	        	...
	        	<div class="col-md-6"><h2>Asignar Médico Tratante:</h2></div>
						...
						<div class="col-md-6" id="dvInfoVentanasOpcionesAsignaMedicoPp"></div>
						...
						<div class="col-md-6">
							<div class="input-group mb-3 col-sm-12">
							    ...
							    <select name="slAsignarMedicoDerivacion" id="slAsignarMedicoDerivacion" class="form-control input-sm select2bs4" onchange="fnCambiaComentarioBitacora(this.value,'<?php echo $qrDerivacion->Fields('N_DERIVACION'); ?>','<?php echo $qrPaciente->Fields('NOMBRE'); ?>','<?php echo $codRutPac; ?>','<?php echo $canasta; ?>')">
							        <option value="">Seleccione...</option>
							        <?php while (!$qrAsignarMedico->EOF) {?>
							          <option value="<?php echo $qrAsignarMedico->Fields('ID') ?>"><?php echo $qrAsignarMedico->Fields('NOMBRE') ?> - <?php echo $qrAsignarMedico->Fields('ESPECIALIDAD') ?> - <?php echo $qrAsignarMedico->Fields('SUBESPECIALIDAD') ?></option>
					...
					<div class="modal-footer" align="right">	
					    ...
					    <button type="button" class="btn btn-info" data-dismiss="modal" onclick="fnAsignarMedicoCasoPp('<?php echo $idDerivacion ?>','<?php echo $idDerivacionPp ?>','<?php echo $tipoUsuario ?>','<?php echo $canasta; ?>')">Asignar Médico Tratante</button> 
			  	</div>     
	  		</div>
...
$('#dvInfoVentanasOpcionesAsignaMedicoPp').load('vistas/modulos/infoVentanasOpcionesPp.php?idDerivacion='+idDerivacion);
...
function fnConsultaSiTieneModuloMedico(){
	...
	$.ajax({
			type:"post",
			data:cadena,
			url:'vistas/modulos/pacientesPp/asignarMedicoCaso/consultaSiTieneModuloMedico.php', 
			success:function(r){
...
function fnAsignarMedicoCasoPp(idDerivacion,idDerivacionPp,tipoUsuario,canasta){
	slAsignarMedicoDerivacion = $('#slAsignarMedicoDerivacion').val();
	...
	cadena = 'idDerivacion=' + idDerivacion +
		...
		$.ajax({
			type:"post",
			data:cadena,
			url:'vistas/modulos/pacientesPp/asignarMedicoCaso/asignarMedicoCaso.php',
			success:function(r){
				if (r == 1) {
					Swal.fire({
					  position: 'top-end',
