...
$query_qrAsignarEnfermeria= "SELECT * FROM $MM_oirs_DATABASE.login WHERE TIPO='3' OR TIPO='2'  order by NOMBRE asc";
...
<form id="formDerivacion">
    ...
    <!-- <form class="form-horizontal" method="POST" action="addEvent.php"> -->
      <div class="row">
        <div class="input-group mb-3 col-sm-3">
            ...
            <input type='text' class="form-control input-sm" name="buscaPacientePp" id="buscaPacientePp" value="<?php echo $codRutPac ?>"/>
        </div>
        <div class="input-group mb-3 col-sm-3">
  ...
  <!--  -->
        <div class="card card-info" id="dvFechasDerivacion">
          ...
          <div class="input-group mb-3 col-sm-4">
              ...
              <select name="slAsignarAdministrativaDerivacion" id="slAsignarAdministrativaDerivacion" class="form-control input-sm">
                  ...
                  <?php while (!$qrAsignarAdministrativa->EOF) {?>
                    <option value="<?php echo $qrAsignarAdministrativa->Fields('USUARIO') ?>"><?php echo utf8_encode($qrAsignarAdministrativa->Fields('NOMBRE')) ?></option>
          ...
          <div class="input-group mb-3 col-sm-4">
              <button id="btnCrearDerivacion" class="btn btn-info"><i class="fas fa-procedures"></i> Crear Derivación</button>
          </div>
        </div>
    </div>
  </div>
...
<script>
  ...
  function fnExtraeTiempoLimite(canasta){
    cadena = 'canasta='+ canasta;
      ...
      $.ajax({
       url: 'vistas/modulos/pacientesPp/crearDerivacion/php/extraeTiempoLimite.php',
       type: "POST",
       data: cadena,
       success: function(r) {
  ...
  $("#buscaPacientePp").rut({formatOn: 'keyup'}).on('rutInvalido', function(e, rut, dv) {
  ...
  // $("#buscaPacientePp").rut({formatOn: 'keyup'}).on('rutValido', function(e, rut, dv) {  
    function fnBuscaPaciente(){ 

        rutPac=$("#buscaPacientePp").val();
        if (rutPac == '') {
          swal("Oops!", "Debe ingresar un rut a la busqueda", "warning"); 
        }else{
          $.ajax({
              url:'vistas/modulos/pacientesPp/crearDerivacion/php/buscaRut.php',
              type:'POST',
              // dataType:'json',
              data:{ nomPac:$('#buscaPacientePp').val()}
  ...
  function fnFiltraPatologias(){
      $("#slTipoPatologiaDerivacion option:selected").each(function () {
          ...
          if (tipoPatologia == '2') {
            ...
            $.post("vistas/modulos/pacientesPp/crearDerivacion/php/filtraPatologias.php",
  ...
  function fnFiltraEtapasPatologias(){
      ...
      $("#slPatologiaDerivacion option:selected").each(function () {        
          ...
          tipoPatologia = $("#slTipoPatologiaDerivacion").val();
            $.post("vistas/modulos/pacientesPp/crearDerivacion/php/filtraEtapasPatologias.php",
            { patologia: patologia },
              function(data){
...
//   }
  ...
  function fnFiltraCanastasPatologias(){
      $("#slEtapaPatologiaDerivacion option:selected").each(function () {
          ...
          $.post("vistas/modulos/pacientesPp/crearDerivacion/php/filtraCanastasPatologias.php",
          { etapaPatologia: etapaPatologia },
            function(data){
  ...
  function fnBuscaDerivacionExistente(patologia){
    codRutPac = $('#buscaPacientePp').val();
    cadena = 'codRutPac='+codRutPac+
             '&patologia='+patologia;

    $.ajax({
     url: 'vistas/modulos/pacientesPp/crearDerivacion/php/buscaDerivacionExistente.php',
     type: "POST",
     data: cadena,
     success: function(r) {
        if (r != 0) {
...
calculaFinFecha = function(){
  ...
  function fnGuardarDerivacion(){
    cadena = $('#formDerivacion').serialize();
      ...
      $.ajax({
       url: 'vistas/modulos/pacientesPp/crearDerivacion/php/guardarDerivacion.php',
       type: "POST",
       data: cadena,
       success: function() {
