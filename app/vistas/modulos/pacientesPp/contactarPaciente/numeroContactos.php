...
$query_qrContactos = "SELECT * FROM $MM_oirs_DATABASE.contacto_paciente_pp WHERE ID_DERIVACION = '$idDerivacion' and TIPO_CONTACTO = '$tipoContacto' order by ID_CONTACTO desc";
        ...
        while (!$qrContactos->EOF) { 
            ...
            <tr>
                ...
                <td><span class="glyphicon glyphicon-user"></span> <font size="3"><?php echo utf8_encode($qrContactos->Fields('NOTA_CONTACTO')); ?></font><br>
                    ...
                    <?php 
                    if ($linkAudio != null) {?>
                        [<a href="#" onclick="$('#dvFrmPlayAudios').load('vistas/modulos/pacientesPp/modulos/contactarPaciente/frmPlayAudios.php?linkAudio='+$('#'+<?php echo $idContacto ?>+'rutaAudioContactos').val())"><span class="badge badge-warning"><i class="fas fa-play"></i></span></a>
                        <a href="#" onclick="document.getElementById('audioContacto').pause()"><span class="badge badge-warning"><i class="fas fa-stop"></i></span></a>]
                    <?php } 

                    if ($rutaDoc != null) {?>
                        <span class=""><a target="_blank" class="btn btn-xs btn-success" href="vistas/modulos/pacientesPp/bitacora/adjuntaDoc/<?php echo $rutaDoc; ?>" ><i class="far fa-file-pdf"></i></a></span>
                    <?php } ?>
