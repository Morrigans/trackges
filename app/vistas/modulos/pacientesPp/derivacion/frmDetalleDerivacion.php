					  			...
					  			<table class="table">
					  				...
					  				<tr>
					  					...
					  					<td>
					  						...
					  						$query_qrCanastaPatologia= "SELECT * FROM $MM_oirs_DATABASE.derivaciones_canastas_pp WHERE ID_ETAPA_PATOLOGIA = '$idEtapa' AND ID_DERIVACION = '$idDerivacion' order by ID_CANASTA_PATOLOGIA ASC";
					  						$qrCanastaPatologia = $oirs->SelectLimit($query_qrCanastaPatologia) or die($oirs->ErrorMsg());
					  						$totalRows_qrCanastaPatologia = $qrCanastaPatologia->RecordCount();
					  						...
					  						<table class="table table-hover">
						  						...
						  						while (!$qrCanastaPatologia->EOF) { 
						  							...
						  							$query_qrBitacora = "SELECT * FROM $MM_oirs_DATABASE.bitacora_pp WHERE ID_CANASTA_PATOLOGIA = '$idDerivacionCanasta' AND ID_DERIVACION='$idDerivacion'";
						  							$qrBitacora = $oirs->SelectLimit($query_qrBitacora) or die($oirs->ErrorMsg());
						  							$totalRows_qrBitacora = $qrBitacora->RecordCount();
						  							...
						  							if ($codTipoPatologia == 1) {
						  								$query_qrCanastaDescPatologia= "SELECT * FROM $MM_oirs_DATABASE.canasta_patologia WHERE CODIGO_CANASTA_PATOLOGIA = '$codCanasta' AND DECRETO = '$decreto'";
					  								...
					  								$query_qrDescPrestadores = "SELECT * FROM $MM_oirs_DATABASE.prestador WHERE RUT_PRESTADOR='$rutPrestador'";
					  							...
					  							?>
			  										<tr> 
			  											...
			  											<!-- <td>
		  													<?php echo utf8_encode($qrDescPrestadores->Fields('DESC_PRESTADOR')) ?>
...
function fnfrmAgregarEtapa(idDerivacion){
	$('#dvfrmAgregarEtapaoCanasta').load('vistas/modulos/pacientesPp/derivacion/modals/frmAgregarEtapa.php?idDerivacion=' + idDerivacion);
}

function fnfrmAgregarCanasta(idDerivacion,etapaPatologia,idEtapaPatologia){
	$('#dvfrmAgregarEtapaoCanasta').load('vistas/modulos/pacientesPp/derivacion/modals/frmAgregarCanasta.php?idDerivacion=' + idDerivacion +'&etapaPatologia=' + etapaPatologia +'&idEtapaPatologia=' + idEtapaPatologia);
}

function fnFinalizarCanasta(idDerivacion,etapaPatologia,idEtapaPatologia,idDerivacionCanasta,prestador){
	$('#dvfrmAgregarEtapaoCanasta').load('vistas/modulos/pacientesPp/derivacion/modals/frmCambiaEstadoCanasta.php?idDerivacion=' + idDerivacion +'&etapaPatologia=' + etapaPatologia +'&idEtapaPatologia=' + idEtapaPatologia +'&idDerivacionCanasta=' + idDerivacionCanasta +'&prestador=' + prestador);
}

function fnMotivoVencimientoCanasta(idDerivacion,etapaPatologia,idEtapaPatologia,idDerivacionCanasta,prestador){
	$('#dvfrmAgregarEtapaoCanasta').load('vistas/modulos/pacientesPp/derivacion/modals/frmMotivoVencimientoCanasta.php?idDerivacion=' + idDerivacion +'&etapaPatologia=' + etapaPatologia +'&idEtapaPatologia=' + idEtapaPatologia +'&idDerivacionCanasta=' + idDerivacionCanasta +'&prestador=' + prestador);
}

function fnAgregarPrestadorCanasta(idDerivacion,etapaPatologia,idEtapaPatologia,idDerivacionCanasta){
	$('#dvfrmAgregarEtapaoCanasta').load('vistas/modulos/pacientesPp/derivacion/modals/frmAgregarPrestadorCanasta.php?idDerivacion=' + idDerivacion +'&etapaPatologia=' + etapaPatologia +'&idEtapaPatologia=' + idEtapaPatologia +'&idDerivacionCanasta=' + idDerivacionCanasta);
}

function fnCargaEliminarCanasta(idDerivacion,etapaPatologia,idEtapaPatologia,canasta,prestador,idBitacora){
	$('#dvfrmAgregarEtapaoCanasta').load('vistas/modulos/pacientesPp/derivacion/modals/frmEliminaCanasta.php?idDerivacion=' + idDerivacion +'&etapaPatologia=' + etapaPatologia +'&idEtapaPatologia=' + idEtapaPatologia +'&canasta=' + canasta +'&prestador=' + prestador+'&idBitacora=' + idBitacora);
}
		...
		$.ajax({
			...
			success:function(r){
				if (r == 1) {
					...
					$('#dvfrmDetalleDerivacion').load('vistas/inicio/inicioSupervisora/modals/derivacion/frmDetalleDerivacion.php?idDerivacion=' + idDerivacion);
	    	}
