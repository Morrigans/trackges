...
echo $query_select = "SELECT * FROM $MM_oirs_DATABASE.etapa_patologia WHERE CODIGO_PATOLOGIA='$codPatologia' AND DECRETO = '$decreto' ORDER BY DESC_ETAPA_PATOLOGIA ASC";
...
?>
 ...
 <?php
while(!$select->EOF){  ?>    
    <option value="<?php echo $select->Fields('ID_ETAPA_PATOLOGIA')?>"><?php echo $select->Fields('DESC_ETAPA_PATOLOGIA')?></option>
<?php
  $select->MoveNext();
  } 
$select->Close();
